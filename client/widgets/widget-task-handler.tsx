...
async function saveLocationToZeke(): Promise<{
  success: boolean;
  locationName?: string;
  error?: string;
}> {
  try {
    ...
    try {
      ...
      if (address) {
        locationName =
          address.street || address.name || address.city || locationName;
      }
    } catch {
    ...
    case "WIDGET_CLICK":
      if (props.clickAction === "SAVE_LOCATION") {
        ...
        if (permStatus !== "granted") {
          ...
          props.renderWidget(
            <Widget status="error" errorMessage="Tap to open app" />,
          );
        ...
        if (result.success) {
          ...
          props.renderWidget(
            <Widget status="saved" lastSaved={result.locationName} />,
          );
        } else {
          await setWidgetData({
            status: "error",
          ...
          props.renderWidget(
            <Widget status="error" errorMessage={result.error} />,
          );
        }
